...
comment on table pomb.country is 'Table with static country data';
comment on column pomb.country.code is 'Primary id and code for country';
comment on column pomb.country.name is 'Name of country';
...
-- Limiting choices for type field on juncture
create type pomb.juncture_type as enum (
  'hike',
  'run',
  'bike',
  'transportation', -- car, train, boat
  'flight'
);
...
create table pomb.juncture (
  ...
  type                pomb.juncture_type not null,
  lat                 decimal not null,
  lon                 decimal not null,
  city                text,
  country             text references pomb.country(code),
  is_draft            boolean,
  marker_img          text,
  created_at          bigint default (extract(epoch from now()) * 1000),
  updated_at          timestamp default now()
...
comment on column pomb.juncture.type is 'Type of juncture';
...
comment on column pomb.post.trip_id is 'Trip the post is related to';
comment on column pomb.post.juncture_id is 'Juncture the post is related to';
comment on column pomb.post.city is 'City the post is related to';
comment on column pomb.post.country is 'Country the post is related to';
